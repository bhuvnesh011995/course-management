...
export default function Department() {
  ...
  const getDepartments = useCallback(async () => {
    try {
      let response = await NewAxiosInstance.get("/constants/department");
    ...
    async (formData) => {
      try {
        ...
        if (response.status === 200) {
          ...
          toast.error("error while added department");
        }
      } catch (error) {
        console.error(error.response);
    ...
    async (id, formData) => {
      try {
        ...
        if (response.status === 200) {
          ...
          toast.error("error while updateing department");
        }
      } catch (error) {
        console.error(error);
  ...
  useCustomUseEffect(getDepartments);
