...
export const AttendanceGenerateModal = ({ isOpen, setIsOpen, tableData }) => {
  ...
  const newGenerateLeadPdf = async (generatedData) => {
    ...
    doc.rect(xPosition, yPosition, -39, 0.2, "F");
    doc.rect(xPosition, 290, -39, 0.2, "F");
    xPosition -= 5;
    doc.setFont("helvetica", "bold");
    xPosition += 1.5;
    yPosition += 1;
    ...
    for (let i = 0; i < tableData.length; i++) {
      ...
      yPosition += 8;
      xPosition += 2;
      doc.text(`${Number(i + 1)}`, xPosition, yPosition, { angle: -90 });

      yPosition += 28;
      doc.text(`${tableData[i].participantName}`, xPosition, yPosition, {
        angle: -90,
      });

      yPosition += 60;
      doc.text(`${tableData[i].participantNric}`, xPosition, yPosition, {
        angle: -90,
      });

      yPosition += 40;
      doc.text(`${tableData[i].coreTradeRegNo}`, xPosition, yPosition, {
        angle: -90,
      });
    ...
    xPosition -= 5;
    yPosition += 2;
    doc.text(`Name & Signature of Trainer(s)/ Date`, xPosition, yPosition, {
      angle: -90,
    });
    xPosition += 5;
    yPosition -= 2;
    ...
    xPosition -= 20;
    yPosition += 2;
    doc.text(`${moment().format("DD-MM-YYYY")}`, xPosition, yPosition, {
      angle: -90,
    });
    xPosition += 20;
    yPosition -= 2;
    ...
    xPosition -= 1;
    doc.rect(xPosition, yPosition, 0.2, 50);
    yPosition += 100;
    doc.rect(xPosition, yPosition, 0.2, 50);
    yPosition -= 100;
    ...
    xPosition -= 5;
    yPosition += 116;
    doc.text(`Company Stamp`, xPosition, yPosition, {
      angle: -90,
    });
    yPosition -= 116;
    xPosition += 26;
    ...
    doc.rect(xPosition, yPosition, -30, 0.2);
    doc.rect(xPosition, yPosition, 0.2, 100);

    const fields = [
      "No. of Tradesmen (exclude absentees):",
      "No. of Foremen (exclude absentees):",
      "No. of Multi-Skilling (exclude absentees):",
      "No. of Direct R1 (exclude absentees):",
      "Total No. Attended and Passed CET:",
    ];

    fields.map((field) => {
      xPosition -= 4;
      yPosition += 1;
      doc.text(field, xPosition, yPosition, { angle: -90 });
      xPosition -= 2;
      yPosition -= 1;
      doc.rect(xPosition, yPosition, 0.2, 100);
    });
